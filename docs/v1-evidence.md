...
| Fail-closed GCS wire classification plus exact append-only request-count, modeled-price, and modeled-latency ratchets for state, file, direct data-plane, and preview pathways | `TestGCSEconomicsProtocolCoverage`, `TestGCSTransferEconomicsProtocolCoverage`, `TestProviderBudgetStateStoreContract`, `TestProviderBudgetTrashAndRestore`, `TestProviderBudgetDurablePreviewStore`, `TestRatchetLedgerOnlyTightensAndCannotDropPathways`; `nix run .#test-provider-budget` |
