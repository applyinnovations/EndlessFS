...
The production binary contains no telemetry, analytics, updater, license check, external identity client, remote theme registry, CDN asset, service worker, SQL client, cache, or queue integration. Required tests construct only explicit loopback control/data listeners. Nix derivation tests use the fixed-output module closure and pinned inputs and have no cloud credential, metadata-service, database, container-daemon, or Internet dependency.
