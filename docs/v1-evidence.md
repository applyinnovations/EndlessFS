...
| Fail-closed GCS wire classification, exhaustive contract/HTTP-route cataloguing, exact append-only request-count/modeled-price/modeled-latency baseline ratchets, explicit lower production-efficiency targets, measured 4 MiB segment envelopes, and composed production-scale evidence for state, file, direct data-plane, preview, and client workflows. Schema-010 measurements are baselines rather than target-conformance evidence. | `TestGCSEconomicsProtocolCoverage`, `TestGCSTransferEconomicsProtocolCoverage`, `TestProviderBudgetCatalogCoversApplicationContractsAndRatchets`, `TestProviderBudgetCatalogClassifiesEveryRegisteredHTTPRoute`, `TestProviderBudgetProductionScaleScenarios`, `TestProviderBudgetProductionScaleTargetsAreStrictlyBetterAndFeasible`, `TestProviderBudgetTargetPayloadEnvelopesAreMeasured`, `TestProviderBudgetStateStoreContract`, `TestProviderBudgetTrashAndRestore`, `TestProviderBudgetDurablePreviewStore`, `TestRatchetLedgerOnlyTightensAndCannotDropPathways`; `docs/provider-request-surface-audit-2026-09-01.md`; `nix run .#test-provider-budget` |
