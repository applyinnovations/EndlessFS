...
| Append-only storage-schema ledger, release validity ranges, immutable fixture for every epoch (including the untagged intermediate), complete ordered suffix from every entry point, old-writer fencing, split-backend support, every-edge crash resumption, resumable and open-ended paged checkpoint inventories, fixed-memory file hashing, legacy checkpoint readability, concurrent starters, upload drain, and corrupt-graph/page denial | `TestStorageSchemaLedgerIsLinearAndAppendOnly`, `TestStorageSchemaLedgerDefinesReleaseValidity`, `TestMigrationEveryRegisteredStorageSchemaOpensAndMutatesWithCurrentCode`, `TestEveryHistoricalReleaseMapsToRegisteredStorageSchemaFixture`, `TestMigrationEveryLedgerEdgeResumesAfterEveryDurableBoundary`, `TestMigrationOldestSchemaTraversesLedgerEdgesInOrder`, `TestMigrationCheckpointInventoryResumesWithoutReReadingCompletedFileObjects`, `TestMigrationCheckpointInventoryRejectsForgedRestartProgress`, `TestMigrationDoesNotReadFileBodiesAgainImmediatelyBeforeOpeningWrites`, `TestCheckpointSupportsInventoryBeyondCanonicalRecordLimit`, `TestCheckpointV1RemainsReadable`, `TestCheckpointVerifierRejectsInvalidBootstrapAndCheckpointRecords`, `TestMigrationReportsStageAndProviderIndependentInventoryProgress`, `TestEightReplicasConcurrentlyMigrateSchema001Fixture`, `TestEightReplicasConcurrentlyMigrateSchema002`, `TestSchema001MigrationResumesAfterUploadRecordUpgrade`, `TestSchema001MigrationWaitsForAndDrainsActiveUpload`, `TestSchema001MigrationRejectsCorruptTrees`, `TestSchema002MigrationRejectsInconsistentPersistedAggregate` |
