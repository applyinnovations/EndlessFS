...
func (e *Engine) quiesceFileOperation(ctx context.Context, key objectstore.Key) error {
	// Schema-008 never creates predecessor FileOperation records. Any preparing
	// record is therefore migration residue with no published visibility and is
	// failed deterministically; only sealed running/committed records are replayed.
	return e.Files().quiesceFileOperation(ctx, key)
}
