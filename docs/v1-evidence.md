...
| Non-public content identity is canonical across replicas and raw-copy cutovers, is preserved by rename/move/trash/restore and same-owner reflink copy, and changes on replacement | shared provider contract `preview content identity lifecycle` over portable memory and local GCS; `TestPortabilityRawCopyPreservesCompleteStateAndContinuesInBothDirections`; `TestResolveGeneratesOnceAndRenameReusesArtifact`; `TestResolveReflinkCopyReusesArtifactAndReplacementDoesNot` |
...
| Requirement | Automated evidence |
|---|---|
| Canonical keys, envelopes, logical versions, domain catalog/heads/pages, owner namespace, retained outcomes, writer/gate, and checkpoint schemas | `TestCanonicalEnvelopeAndLogicalVersion`, `TestCanonicalKeyLayoutAndBounds`, `TestConsistencyDomainHeadCorruptionFailsClosed`, `TestNamespaceAndOutcomePageCorruptionFailsClosed` |
...
| Conditional domain-head publication, unrelated-key contention retry, same-key one-winner denial, lost-success recovery, stale-writer denial, helpable cross-domain decisions, catalog freeze, and writer compatibility | `TestConsistencyDomainDistinctConcurrentMutationsRetryUnrelatedHeadRace`, `TestConsistencyDomainConflictingConcurrentMutationsKeepOneWinner`, `TestConsistencyDomainLostHeadCommitResponseRecoversInSameCall`, `TestConsistencyDomainFreezeWinsAgainstPausedStaleWriter`, `TestConsistencyDomainEightReplicaConflictHasOneWinner`, `TestPortableCrossDomainTransitionRecoversLostDecisionAndEightReplicaReplay`, `TestPortableCrossDomainTransitionRestartsAfterEveryDurableBoundary`, `TestCheckpointHelpsPendingTransitionBeforeFreezingDomains`, `TestCheckpointFreezeClosesEveryRegisteredDomainAndRegistrationRace`, `TestReplicaCompatibilityRejectsWriterConfigurationDrift` |
...
| Metadata-only smart-upload planning, size-first ambiguity filtering, stale/cross-owner denial, incremental live-projection maintenance, cardinality-independent warm GCS request budgets, and browser MD5+CRC32C one-pass hashing | `TestUploadPlanningUsesSizeBeforeExactProviderFingerprint`, `TestUploadPlanningDetectsExactTargetAndRejectsCrossOwnerToken`, `TestUploadPlanningRejectsStaleSnapshotsAndIncrementallyRemovesTrashedFiles`, `TestUploadPlanningWarmRequestBudgetIsIndependentOfBatchCardinality`, `TestIntegrationUploadPlanningRoutesAreStrictOwnerScopedMetadataQueries`, `TestE2EUploadPlannerHashesMD5AndCRC32CInOneWorkerPass`; `docs/smart-upload-planning-evidence.md` |
...
| Eleven-epoch append-only storage-schema ledger, release validity ranges through v0.7.0, immutable fixture for every epoch/profile, predecessor-produced complete application corpora, exact `009 -> 010` state-conservation receipts and activation barrier, feature-only authority-verified `010 -> 011` activation, real post-migration passkey authentication, complete ordered suffix through schema 011, split-backend support, every-edge crash resumption, concurrent starters, and corrupt source/proof/target denial | `TestStorageSchemaLedgerIsLinearAndAppendOnly`, `TestConservationEraMigrationCannotActivateWithoutAuthorityVerifier`, `TestStorageSchemaReleaseLedgerDefinesDerivedValidityRanges`, `TestMigrationEveryRegisteredStorageSchemaOpensAndMutatesWithCurrentCode`, `TestApplicationWriterProfilesOpenSchema011Fixtures`, `TestMigrationSchema009ProductionResidueRestoresRealPasskeyAuthentication`, `TestMigrationSchema009RecoversIndexedApplicationStateDroppedBySchema008`, `TestMigrationSchema010FailsClosedForConflictingCurrentState`, `TestMigrationSchema010FailsClosedWhenIndexedVersionIsMissing`, `TestMigrationSchema010RejectsCorruptDurableConservationReceiptOnRestart`, `TestMigrationSchema010ConcurrentReplicasConvergeWithIndexedState`, `TestEveryHistoricalReleaseMapsToRegisteredStorageSchemaFixture`, `TestMigrationEveryLedgerEdgeResumesAfterEveryDurableBoundary`, `TestMigrationOldestSchemaTraversesLedgerEdgesInOrder`, `TestMigrationCheckpointInventoryResumesFromBoundedPagesWithoutReadingFileBodies`, `TestMigrationDoesNotReadFileBodiesAgainImmediatelyBeforeOpeningWrites`, `TestEightReplicasConcurrentlyMigrateSchema001Fixture`, `TestGarbageCollectionTerminalMarksProtectLaggingSweeper`, `TestConsistencyDomainHeadCorruptionFailsClosed`, `TestNamespaceAndOutcomePageCorruptionFailsClosed`; `nix run .#test-migration -- v0.7.0` |
| Cross-replica upload idempotency, 10,000-item packed admission/completion/cancellation, authenticated 1,000-item crash progress, compact whole-batch abort overlay, stable browser ledger/batch replay, ancestor-root contention retry, lost-success reconciliation, completion/abort one-winner convergence, capability drain, and resumability | `TestPortableUploadInitiationIsIdempotentAcrossReplicas`, `TestConcurrentReplicaUploadInitiationHasOneIdempotentOutcome`, `TestEightReplicaDistinctUploadInitiationsSurviveSharedHeadContention`, `TestIntegrationBatchUploadItemIdempotencySurvivesEnvelopeRetry`, `TestProviderBudgetUploadBatchTenThousandLifecycle`, `TestPortableUploadBatchAbortIsAtomicAndReplayable`, `TestConcurrentCompletionAndCompactBatchAbortHaveOneAtomicWinner`, `TestUploadBatchAbortProgressRestartBoundsRepeatedProviderWork`, `TestSchema011UploadBatchAbortOverlayCorruptionFailsClosed`, `TestE2EBrowserBootstrapLoginDriveShareAndTrash`, `TestEightReplicaConcurrentMultiFileCompletionConvergesRecursiveAggregates`, `TestUploadCompletionLostSuccessIsIdempotentlyReconciled`, `TestCheckpointWaitsForActiveCapabilityThenDrainsItAfterExpiry` |
...
| Fail-closed GCS wire classification, exhaustive contract/HTTP-route cataloguing, exact append-only request-count/modeled-price/modeled-latency ratchets, measured 4 MiB segment envelopes, and composed target-conformance evidence for state, file, direct data-plane, preview, and client workflows. Schema-010 measurements remain historical baselines; real schema-011 events meet every production-scale target. | `TestGCSEconomicsProtocolCoverage`, `TestGCSTransferEconomicsProtocolCoverage`, `TestProviderBudgetCatalogCoversApplicationContractsAndRatchets`, `TestProviderBudgetCatalogClassifiesEveryRegisteredHTTPRoute`, `TestProviderBudgetProductionScaleScenarios`, `TestProviderBudgetProductionScaleScenariosConformToTargets`, `TestProviderBudgetTargetPayloadEnvelopesAreMeasured`, `TestProviderBudgetStateStoreContract`, `TestProviderBudgetTrashAndRestore`, `TestProviderBudgetUploadBatchTenThousandLifecycle`, `TestProviderBudgetVisiblePreviewWindowUsesOneReadyCatalogRead`, `TestProviderBudgetCheckpointGarbageCollection128Objects`, `TestProviderBudgetDurablePreviewStore`, `TestRatchetLedgerOnlyTightensAndCannotDropPathways`; `docs/provider-request-surface-audit-2026-09-01.md`; `nix run .#test-provider-budget` |
...
| Duplicate-aware upload strategy selection, immediate release of unique-size files, bounded two-worker one-pass MD5+CRC32C hashing for ambiguous local files, metadata-only same-owner reuse, and refresh/connection-loss-safe planning state | `TestE2EUploadPlannerHashesMD5AndCRC32CInOneWorkerPass`; `TestBrowserSourceKeepsSecretsEphemeralAndUntrustedTextOutOfHTML`; `TestUploadPlanningUsesSizeBeforeExactProviderFingerprint`; `TestUploadPlanningWarmRequestBudgetIsIndependentOfBatchCardinality`; `docs/smart-upload-planning-evidence.md` |
...
| Bounded large-directory list/grid DOM, exactly two requests (10,000 + 3 entries) for the 10,003-entry fixture, bounded preview requests, zero redundant terminal-operation polls, zero provider calls for 10,000 restored transfer records, and deterministic filter work | `TestE2EBrowserBootstrapLoginDriveShareAndTrash`, `TestE2ERestoringTenThousandTransfersDoesNotBlockWorkspaceOrPollUploads`; `nix run .#test-ui-benchmark` (`ui-benchmark-v1` JSON-line evidence) |
...
| AC-023 | Canonical format golden/key/bounds tests plus strict writer, gate, domain catalog/head/page, state-version, namespace, retained-outcome, idempotency, and checkpoint codecs. |
...
| AC-029 | Catalog-registration/freeze races, paused-writer denial, interrupted-reopen recovery, active-capability drain, exact reachable inventory, and read-only verifier tests. |
...
| AC-080 | Independent two-through-eight-replica state, namespace, upload, migration, freeze, and retained-outcome tests; no engine coordination is process-local. |
| AC-081 | `TestConsistencyDomainFreezeWinsAgainstPausedStaleWriter`, `TestFrozenCatalogRejectsNewDomainWithoutPublishingValues`, and `TestCheckpointFreezeClosesEveryRegisteredDomainAndRegistrationRace`. |
| AC-082 | `TestConsistencyDomainLostHeadCommitResponseRecoversInSameCall`, `TestConsistencyDomainSameMutationConcurrentCallersObserveOneOutcome`, and `TestNamespaceMoveLostSuccessAndEightReplicaReplayConverge`. |
| AC-083 | `TestConsistencyDomainFreezeWinsAgainstPausedStaleWriter` and `TestConsistencyDomainEightReplicaConflictHasOneWinner` resume stale contenders against a changed/frozen head condition. |
| AC-084 | `TestEveryNamespaceMutationPublishesNothingBeforeHeadCommit`, namespace recursive-operation tests, and eight-replica replay/convergence tests. |
| AC-085 | Schema-008 direct-upload initiation/completion/lost-success/expiry/abort/checkpoint-drain tests and `TestLogicalCopyAndUploadPublicationUseNoFileBackendCopy`. |
...
| AC-087 | `TestCheckpointFreezeClosesEveryRegisteredDomainAndRegistrationRace`, `TestStartupFinishesInterruptedCheckpointReopen`, and active-capability tests require safe freeze/reconciliation/drain before checkpoint closure. |
...
| MP-012 | Provider lifecycle contract plus rename/reflink-copy reuse and replacement distinct-generation service tests. |
