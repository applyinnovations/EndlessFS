...
The root directory ID is a fixed format constant. Every other directory and blob ID is a stable opaque random identifier stored in canonical records. Each mutable `directory.json` root points to one immutable manifest whose constant-size header names a persistent bounded fan-out name-search root and fixed roots for modified, size, and kind order. Immutable leaves contain uniquely ordered child entries; branches carry bounded ranges, while the name tree also carries exact byte/file/count aggregates. Lookup and pagination read O(log D + page size) in every supported order, and one entry change structurally shares unchanged nodes while rewriting O(log D) nodes in each fixed index per affected ancestor. Continuations bind the exact immutable manifest and last composite sort key, not an offset or entry snapshot. The root contains its logical revision, non-negative persisted recursive-byte and recursive-file-count aggregates, an order-independent content accumulator and structural digest, and any operation-owned pending pre/post transition; the selected manifest repeats these values so `Stat` can verify them without reading tree leaves. Historical page-ID manifests remain migration inputs only. A stored name MUST hash to its recorded digest and match the requested name; a mismatch is a collision or corruption and fails closed.
...
A single-directory content change writes only changed immutable search-tree nodes plus a new manifest and conditionally replaces `directory.json`; that root replacement is its visibility point. Directory mutations apply entry deltas to recursive counts and the content accumulator and never reconstruct an unchanged directory. Unreferenced nodes or manifests are garbage, never visible entries. A multi-directory operation first CAS-locks every affected root in canonical resource-ID order, records immutable pre/post manifests, aggregates, and current fence in each transition, and then conditionally changes the durable operation record to `committed`. Its roots, copy instructions, and prerequisite references are stored in bounded hash-chained step pages; full prerequisite bodies live once in immutable operation staging and are promoted with digest verification. No operation or admission envelope embeds a collection or body set that grows with a subtree. Readers encountering a pending transition use the pre-state before commit and the post-state after it. Root finalization and closed-gate garbage collection are idempotent consequences of durable state.
