...
Per-group reclaimable bytes count all but one occurrence of that exact group. Do not sum directory-group savings with descendant file-group savings because they overlap. The `/duplicates` browser presents one group action at a time, requires an explicit keeper for exact copies, previews at most 100 common files per reconciliation, keeps unique files untouched, and presents exact-group and directory-pair ignores in collapsed sections.
