...
The clarified provider-agnostic v1 contract requires the canonical provider-independent single-/split-bucket storage-set format, portable logical versions, one portable storage engine over thin object-store backends, safe multi-replica atomic publication/freeze/recovery, and verified quiescent raw-copy portability in spec sections 5, 8, 9, 18, 21, 22.3, and 22.4. Schema 008 is the current ordinary runtime: owner namespace and routed application state use canonical consistency-domain heads, structurally shared immutable pages, retained outcomes, catalog freeze, and metadata-only checkpoints. Schema-007 runtime machinery is retired; its private decoders exist only for the adjacent migration. Implemented claims still require evidence for every relevant checklist item; the repository MUST NOT be described as v1 feature complete while any required item remains unchecked.
...
8. Transformations are forward-only, deterministic, idempotent, crash-resumable, and safe under mixed-version replicas. They quiesce through the source epoch's canonical gate or an approved replacement freeze/commit protocol, use portable records plus native CAS only for the immediate conditional operation, fail closed on ambiguity or corruption, and do not publish partial logical state. Schema 010 begins the conservation era: every incoming edge to schema 010 or later MUST register a migration-specific pre-activation authority verifier, and the shared writer-set activation path MUST execute it while the gate and target authority remain frozen. A runner returning success or writing target feature markers can never substitute for that proof.
9. `nix run .#test-migration` and the `migration` flake check are mandatory pull-request gates. They MUST execute the complete owning migration packages without a permissive test-name filter, the epoch/profile matrix, predecessor-interruption fixtures, real application writer-profile startup tests, candidate release mapping locally, and enforce at least 98% aggregate statement coverage across `internal/portable/migration.go`, every numbered production migration implementation, and `internal/portable/migration_ledger.go`. A source-file matcher MUST include future numbered migration files by construction rather than enumerating selected epochs. The percentage is a backstop, not permission to omit explicit boundary, semantic-oracle, denial, restart, concurrency, or fault-injection assertions. Release CI runs the same `nix run .#test-migration -- "$release_tag"` command before building or publishing artifacts; CI is confirmation, never the first place a migration profile is exercised.
...
- Give every mutation one provider-independent conditional linearization point that races totally with checkpoint freeze and incompatible writers. Schema 008 uses the exact consistency-domain head CAS; catalog registration makes the complete mutable-head set freezeable. Candidate-ticket admission is historical migration input and must not return to the ordinary path without new comparative evidence and a reviewed schema change. Process-local maintenance state, a leader, load-balancer draining, sticky routing, or a grace period cannot be correctness dependencies.
...
`docs/storage-schema-008-implementation.md` records the selected architecture,
migration, guarantee proof, and measured implementation ratchets. The earlier
proposal and experiment remain decision evidence, not an alternate runtime
contract.
...
Multi-replica tests use two through eight separately constructed engines and a deterministic scheduler that can pause, crash, partition, restart, and resume them at every durable boundary declared by the applicable protocol. For schema 008 that includes domain-head read/CAS, immutable page publication, lost provider response, compaction, catalog registration/freeze, upload capability drain, gate transition, migration cutover, checkpoint construction, and unfreeze. Historical admission/operation boundaries remain in migration tests only. The schedules prove one-winner publication, stale-writer denial, lost-success recovery, complete visibility, compatibility rejection, and no permanent lock without wall-clock sleeps.
