...
| Persisted recursive-byte aggregates at every directory and area root, with constant-metadata-read lookup, upload/replacement/move/copy/trash/restore/delete maintenance, pre/post-commit visibility, overflow/corruption denial, and raw-copy continuation | `TestPortableRecursiveByteAggregatesTrackEveryFileMutation`, `TestPortableRecursiveAggregateStatDoesNotReadManifestPages`, aggregate assertions in `TestReplicaDropAfterRootPrepareRecoversAtOneCommitPoint` and `TestPortabilityRawCopyPreservesCompleteStateAndContinuesInBothDirections`, `TestPortableDirectoryManifestCorruptionMatrixFailsClosed`, `TestPortableDirectoryEntryValidationMatrix` |
