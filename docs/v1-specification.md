...
Every epoch MUST have one immutable raw state/file-backend fixture produced by code that actually wrote that epoch and bound to a source tag or commit plus a hard-coded SHA-256 digest. This includes untagged intermediate epochs. Release-to-epoch validity ranges are recorded separately in the production ledger; multiple releases MAY share an epoch only when their durable bytes and semantics are identical. Current-code reconstructions, hand-edited approximations, decoder-only tests, and already-migrated fixtures are not migration evidence.

A current binary MUST import every epoch fixture into fresh single- and split-backend stores, resolve and execute the entire remaining ordered ledger suffix, verify the full authoritative result, and successfully perform a post-upgrade mutation. Every adjacent edge MUST prove deterministic restart at each durable boundary and two-to-eight-replica convergence; the composed oldest-to-current path MUST prove exact edge order. Corrupt, truncated, mixed, newer, unreachable, or semantically inconsistent state fails closed. Transformations are forward-only, gate-quiesced, fenced, deterministic, idempotent, crash-resumable, and provider-independent; ordinary runtime decoders remain strict.
...
17. The mandatory migration matrix covers every append-only storage-schema epoch with an immutable producer fixture, traverses every remaining adjacent edge from each entry point, passes after the change, and maps the candidate release into a declared validity range before publication.
