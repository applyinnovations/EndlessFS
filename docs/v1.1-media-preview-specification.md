...
**Status:** Implemented with deterministic memory and locally protocol-qualified durable GCS preview stores
...
EndlessFS v1.1 adds an always-available media-browsing grid and full-viewport viewer alongside the existing file list and safe original-file preview behavior. Generated image thumbnails are optional and disposable.
...
- an always-available thumbnail grid alongside the existing file list;
...
- deterministic portable-source, generator, HTTP, browser, security, race, and failure tests with no cloud account.

### 2.2 Built-in v1.1 generation support

The base v1.1 build MUST generate previews for portable-engine-validated raster image formats supported by its closed generator registry. The required set is PNG, JPEG, GIF, WebP, DNG, Canon CR2/CR3, Fujifilm RAF, Nikon NEF, Olympus ORF, Panasonic RW2, Pentax PEF, and Sony ARW. The exact registered RAW media types are `image/x-adobe-dng`, `image/x-canon-cr2`, `image/x-canon-cr3`, `image/x-fuji-raf`, `image/x-nikon-nef`, `image/x-olympus-orf`, `image/x-panasonic-rw2`, `image/x-pentax-pef`, and `image/x-sony-arw`; arbitrary `image/*` values remain unsupported. Animated formats produce a static representative frame and MUST NOT produce an animated preview artifact.
...
The existing authoritative `provider.Storage` interface MUST NOT gain mandatory preview methods. Preview source reads use its exact-version stat/download operations. The independently optional artifact store remains a narrow v1.1 port, so a runtime without generated thumbnails continues to satisfy the v1 storage contract.

Suggested packages are:

```text
internal/preview                  policy, orchestration, recipes, artifact interface, and contracts
internal/preview/imagegen         closed built-in image generator
internal/preview/memory           independently removable deterministic preview store
internal/preview/durable          provider-neutral durable preview semantics over conditional objects
internal/objectstore              shared thin memory/GCS transport and direct-capability backends
internal/provider                 existing exact-version authoritative source interface
internal/httpapi/previews.go      authenticated preview transport
internal/web                     virtualized grid and full-viewport viewer
```

### 3.1 Authoritative source use
...
- provider-neutral integrity verification of an immutable artifact without requiring a control-plane body read when the selected backend can attest from native integrity metadata;
...
|---|---|---|
| `.#container` | `image` | The existing minimal container plus the built-in image generator and pinned LibRaw helper. |
...
Media browsing has no feature flag: the grid, metadata filters, file-type icons, and full-viewport viewer are always available. Supplying the removed `ENDLESSFS_MEDIA_BROWSER_ENABLED` variable is a startup error with a migration message; only generated thumbnails are configured below.

| Variable | Required/default | Behavior |
|---|---|---|
| `ENDLESSFS_PREVIEW_PROVIDER` | Default `disabled` | Accepts `disabled`, the deterministic loopback `mock`, or durable shared `gcs`. |
| `ENDLESSFS_GCS_PREVIEW_BUCKET` | Required with preview provider `gcs` | Private disposable preview bucket, distinct from both configured authoritative bucket roles. |
...
| `ENDLESSFS_PREVIEW_KEY_SECRET` | Required with preview provider `gcs`; ephemeral default for `mock` | Canonical 256-bit base64url key used only for non-revealing artifact identity derivation; stable and identical across replicas sharing the preview bucket. |
...
The deterministic `mock` preview provider is a local proof implementation, not a deployable preview bucket. Configuration MUST reject it unless the control plane uses an HTTP loopback origin with the mock authoritative-storage provider. HTTPS, public listeners, GCS authoritative storage, and multi-process deployment use the durable shared `gcs` preview provider or keep generated previews disabled; they MUST NOT start with process-local preview storage and a loopback artifact origin.

The durable preview store owns provider-neutral claim, epoch, immutable-generation, manifest, visibility, validation, and retention semantics. It depends on the same narrow conditional-object and direct-transfer interfaces used by authoritative storage. The GCS profile only selects a distinct backend instance and signing configuration; it does not duplicate filesystem, state, or GCS transport logic. Preview writes are exclusively server-side. Browser access uses short-lived signed `GET` capabilities and exact-origin preview-bucket CORS for `GET` and `HEAD`.

The active preview provider/store identity, configured capability profile, resolutions, policy, provider-independent preview-key identifier, and integrity protocol participate in replica compatibility. Native bucket names and secrets do not enter the writer-set record. Replicas configured for different preview stores, keys, or active capabilities fail compatibility before serving together.
...
When `ENDLESSFS_PREVIEW_PROVIDER` is not `disabled`, startup MUST perform a store validation within the same timeout. It proves the configured preview bucket/store exists and that the application can authenticate, list its reserved preview prefix, conditionally create a probe generation containing an embedded fixed one-pixel WebP, read and validate that WebP, commit and read its immutable manifest, and issue and exercise an exact-artifact capability. The exercised response must have exact WebP content type, safe inline disposition, `no-store`, and the expected bytes. Cleanup of the opaque probe is best effort because bucket retention may prohibit deletion. Failure of any required operation is a startup error. The error identifies the failed permission category without printing a bucket name, object key, credential, capability, or probe identity.
...
The portable storage engine exposes a non-public preview content identity containing:

```text
contentID          opaque identity deterministically derived from the canonical blob identity
contentVersion     opaque revision derived from canonical rendering-relevant content fields
contentModifiedAt  authoritative time of the content revision
```

`contentID` and `contentVersion` are provider-independent domain values derived from the existing canonical blob identity and immutable content fields; they are never additional fields in canonical directory records and never appear in public HTTP responses. Upload completion assigns them. Rename, move, directory-tree move, trash, restore, and same-owner reflink copy preserve them because those operations preserve the blob identity. Replacement creates a new blob identity and therefore assigns a new `contentID` and `contentVersion`. Delete followed by a later new upload also assigns a new `contentID`. This derivation keeps pre-preview buckets byte-canonical, requires no state migration, and remains deterministic after a raw bucket copy to another provider. Thin object-store adapters MUST NOT derive, interpret, or persist these values in provider custom metadata. Client-supplied values are never accepted.

Canonical file records created before v1.1 require no preprocessing. The portable engine derives identity on every read from the canonical blob identity and rendering-relevant canonical fields, never from a provider generation, ETag, bucket, custom metadata, or local process state. Reads, moves, and reflink copies remain side-effect free with respect to identity; no v1.1 operation materializes extra directory fields. Rename, move, same-owner copy, trash, restore, replica takeover, and raw-copy cutover therefore retain the same semantics as newly created records.
...
contentID
contentVersion
portable-engine-validated media type
source byte size
...
The durable artifact store maps that binding and each generation ID to provider-internal names using domain-separated HMAC-SHA-256 digests. Its versioned private layout is:

```text
preview/v1/b/<binding-digest>/head.json
preview/v1/b/<binding-digest>/g/<generation-digest>/manifest.json
preview/v1/b/<binding-digest>/g/<generation-digest>/artifact.webp
```

This layout is not a public API or a provider-specific contract. The bounded head contains the active claim epoch and a bounded history of committed artifact metadata. Manifests and names MUST NOT contain raw user IDs, filenames, virtual paths, entry versions, content identities, bucket names, or provider keys. Native object versions are used only for the immediate conditional request and are never written into a record.
...
Renaming, moving, or same-owner reflink-copying a file or directory tree preserves each file's content identity, so compatible preview artifacts are immediately reusable at the new authorized path without regeneration. Trash makes artifacts inaccessible; restore reuses them when the content identity is unchanged. Replacement changes the content version and therefore requires a new preview. Every preview capability request independently reauthorizes the current live path and exact entry version, so artifact reuse never grants access through the source path. None of these original-file operations waits for preview work. Inaccessible or superseded artifacts may be removed later by bucket lifecycle.
...
Generation operation records and hashed idempotency bindings use the existing portable conditional state store, not process memory. They are owner-scoped, retained for a bounded 24-hour default window, and survive replica restart or replacement. A request that encounters an unexpired running lease returns the same operation ID; after lease expiry it may CAS-take over that same operation with a higher epoch. Expired terminal records and their idempotency bindings are removed incrementally under bounded per-request cleanup.
...
Concurrent requests for the same content binding, recipe, and requested variant MUST share one store-claimed generation attempt across every EndlessFS replica using that preview store. Process-local coalescing is an optional optimization only. The artifact store uses conditional claims with a bounded lease and monotonically increasing epoch; a stale worker cannot commit after takeover. Duplicate completed artifacts remain semantically harmless for recovery, but they are not the concurrency-control strategy.

A durable follower operation that encounters an existing claim records the exact opaque generation claim it is awaiting. Polling reports success only after that exact immutable generation is committed and the current source path/version is reauthorized; it MUST NOT mistake a pre-existing artifact for completion of a concurrent Regenerate request. Polling is read-only. A mutation retry retains the original idempotency key and may take over the same operation after its lease expires.
...
- record exact content type, dimensions, byte size, SHA-256, CRC32C, and recipe ID in the manifest.

The manifest CRC32C is an application-computed provider-independent value. Before issuing an artifact capability, the durable store asks the object backend to verify the expected size and CRC32C and uses the exact returned object incarnation for the capability. GCS satisfies this operation from its stored CRC32C metadata; another backend may compute the same checksum from bytes. GCS checksum encodings and generations remain adapter-local and are never written to the manifest or head record.
...
Grid images use explicit dimensions, asynchronous decoding, low fetch priority, and lazy requests. The browser keeps capability values and object URLs in memory only, aborts requests for evicted tiles, and releases object URLs when tiles or viewers close. Grid eviction discards any `ready` cache state that depended on a revoked object URL so a re-entering tile reacquires an exact-generation capability. A viewer MAY retain only the already-validated WebP blob bytes in bounded document memory until the exact capability that authorized those bytes expires. Such reuse is keyed by source path, portable version, and variant; the cache record retains the validated artifact metadata required to reconstruct the image element. It creates a fresh object URL on reopen, never retains the provider capability, never extends authorization beyond capability expiry, and is cleared on logout or document destruction.

Preview capability and artifact responses use `Cache-Control: no-store` unless a later privacy specification explicitly defines an encrypted/private cache. The browser fetches the capability without navigating to it, requires exact `image/webp`, verifies the exact manifest byte size and SHA-256, validates the RIFF/WebP signature, and only then constructs a new typed in-memory blob for display. Fast loading is achieved through small variants, direct preview-provider transfer, virtualization, bounded parallelism, and reuse within the current document—not through persistent sensitive browser caches.
...
|---|---|---|
| `POST` | `/api/v1/previews/resolve` | Resolve a bounded batch of visible exact path/version/variant requests and start at most one policy-eligible automatic generation per request. |
...
The existing list and media grid are always available, with the list as the default. A labeled list/grid choice controls presentation without changing authorization or listing semantics. No environment variable or preview-provider setting may hide or disable the grid, metadata filters, file-type icons, or full-viewport viewer.
...
- re-resolves a `generating` tile with bounded backoff while it remains rendered, then exposes a stable failure fallback rather than freezing indefinitely; and
...
The application, not the theme, decides the semantic category, accessible name, fallback order, dimensions, loading behavior, and interaction. Theme API 2.0 built-ins are complete, and same-version custom themes inherit omitted slots from their immutable 2.0 parent. Theme API 1.x bundles are rejected rather than adapted.
...
- retains one idempotency key across bounded polling and mutation retries, including recovery after a lost response or expired owner lease;
...
v1.1 filtering applies to directory entries already loaded through the ordinary paginated listing. Supported predicates include filename text, entry kind, portable-engine-validated media category, byte-size range, modification-time range, and preview state.
...
- generated artifacts are served only as exact `image/webp`, safe inline disposition, `no-store`, and no content negotiation from untrusted request values; application-owned responses add `nosniff`, while direct-provider responses are fetched rather than navigated and are accepted only after exact type and RIFF/WebP signature validation;
...
Preview generation is a distinct bounded processing plane. File bytes still never enter ordinary HTTP control handlers. The image profile executes each codec operation in a one-shot worker mode of the same Go binary. The parent passes a bounded internal protocol through pipes, supplies no credentials or capability secrets in process arguments or environment, and terminates the worker when the hard operation deadline expires. Camera RAW requests may invoke the pinned LibRaw `dcraw_emu` helper from inside that one-shot worker with fixed arguments only. On Linux, input is passed through a sealed anonymous memory file and the helper receives parent-death termination; source paths, filenames, commands, provider capabilities, and decoder output never enter logs or public errors. The helper's bounded PPM output is parsed by the Go worker, dimension/pixel checked, resized, and encoded to the same metadata-free static WebP artifact. No runtime-selected executable, argument, plugin, or downloaded codec is allowed. A later profile may define another closed, packaged implementation with equivalent isolation.
...
2. a provider-neutral durable preview store over the thin conditional-object boundary;
3. distinct loopback preview data-plane origins for memory and credential-free GCS protocol qualification;
4. the same artifact-store contract suite over memory and the local GCS protocol fake;
5. deterministic multi-replica claim, takeover, stale-worker, lifecycle, corruption, and lost-success tests;
6. a deterministic fake generator for orchestration and fault tests;
7. reviewed image fixtures for the real built-in image generator;
8. injected clock, IDs, claim schedule, failures, and ordering;
9. instrumentation for source bytes read, preview bytes written/read, and control-handler bytes; and
10. removal/recreation controls that do not mutate the authoritative portable source backend.
...
- source replacement, rename, move, copy, trash, restore, delete, and disabled owner, including proof that rename/move/reflink-copy reuse the same artifact without generator calls;
...
- WebP-only output, signature, metadata stripping, orientation, dimensions, alpha, source-aspect preservation, deterministic recipe behavior, and valid/malformed camera RAW decode boundaries;
...
Any image encoder/decoder dependency is pinned, vendored or supplied through locked Nix inputs, license-reviewed, vulnerability-scanned, and recorded in `docs/dependencies.md` plus the release inventory. It MUST be either statically linked into the base binary or packaged as an immutable helper in the same Nix/OCI closure under the one-shot isolation contract above. The standard library is preferred; a third-party codec requires the repository’s normal maintenance, license, security-history, and necessity justification.
...
- the locally qualified GCS source and durable-preview boundaries, absent live-GCS validation, and absent production preview-bucket/deployment validation.

---

## 14. Acceptance criteria

**MP-001** — With all preview settings absent, generated thumbnails are disabled while existing v1 file, safe-original preview, share, test, and release behavior remains unchanged.
**MP-002** — Without a preview provider, every authenticated file browser supplies the virtualized grid, full-viewport navigation, metadata filtering, and complete icons without source reads or a feature flag.
**MP-003** — Preview storage is an independent interface with deterministic memory and durable shared object-store implementations; configured access is fully validated at startup, runtime loss makes readiness unhealthy, and original list, stat, upload, download, move, copy, trash, restore, and share data remains safe and operable.
...
**MP-007** — PNG, JPEG, GIF, WebP, and the exact registered DNG/CR2/CR3/RAF/NEF/ORF/RW2/PEF/ARW inputs produce validated static WebP-only artifacts with the required metadata removal, orientation, source aspect ratio, dimensions, and bounded recipes.\
...
**MP-012** — Rename, move, and same-owner reflink copy preserve content identity and immediately reuse compatible artifacts without regeneration; replacement uses a distinct identity.
...
**MP-015** — Documentation and release evidence distinguish deterministic memory proof, credential-free durable GCS protocol qualification, live GCS validation, and production deployment claims.
...
- [x] Media browsing is unconditional; generated-thumbnail provider configuration is independent and defaults off.
...
- [x] Canonical portable records expose non-public move-stable content IDs/versions and preserve them across replicas, raw-copy cutovers, rename, move, trash, and restore without provider metadata.
...
- [x] File-type icons are complete across the Theme API 2.0 built-ins and inherit safely within the same-version parent contract; Theme API 1.x is rejected.
...
- [x] Durable preview semantics pass the shared memory/GCS contract, multi-replica fencing, corruption, lifecycle, recovery, and security-sensitive coverage gates.
- [x] Release evidence distinguishes locally qualified GCS source/preview access from unperformed live/production validation.
