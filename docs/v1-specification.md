...
The root directory ID is a fixed format constant. Every other directory and blob ID is a stable opaque random identifier stored in canonical records. Each mutable `directory.json` root points to one immutable manifest whose constant-size header names a persistent bounded fan-out name-search root, fixed roots for modified, size, and kind order, and a descendant-content root ordered by `(file group ID, relative path)`. Immutable leaves contain uniquely ordered entries and branches carry bounded ranges, counts, and a fixed 16-position MinHash summary over their confirmed file-group set; parent summaries are the component-wise minima of their children, so insert and delete rewrite only the search path. The name tree also carries exact byte/file/count aggregates, while the content-tree root count equals the recursive file count. Lookup and pagination read O(log D + page size) in every supported order, and one file change structurally shares unchanged nodes while rewriting O(log D + log F) nodes in each affected ancestor. Continuations bind the exact immutable manifest and last composite key, not an offset or entry snapshot. The root contains its logical revision, non-negative persisted recursive-byte and recursive-file-count aggregates, an order-independent content accumulator and structural digest, and any operation-owned pending pre/post transition; the selected manifest repeats these values and pins the content sketch so `Stat` can verify them without reading tree leaves. Historical page-ID manifests remain migration inputs only. A stored name MUST hash to its recorded digest and match the requested name; a mismatch is a collision or corruption and fails closed.
...
A single-directory content change writes only changed immutable search-tree nodes plus a new manifest and conditionally replaces `directory.json`; that root replacement is its visibility point. Directory mutations apply entry deltas to recursive counts and the content accumulator and never reconstruct an unchanged directory. Unreferenced nodes or manifests are garbage, never visible entries. A recursive operation or non-empty directory replacement first admits a bounded durable `preparing` header before traversing its source. The header pins the request, gate, and selected roots; authenticated bounded preparation runs carry traversal progress and sorted prerequisite/copy/catalog/content deltas. Fixed-fan-in provider-backed merges seal those runs directly into bounded hash-chained operation-step pages. Another replica can resume from the last completed run without rebuilding it, and any changed pinned root fails before visibility preparation. The sealed multi-directory operation then CAS-locks every affected root in canonical resource-ID order, records immutable pre/post manifests, aggregates, and current fence in each transition, and conditionally changes the durable operation record to `committed`. Full prerequisite bodies live once in immutable operation staging and are promoted with digest verification. No operation or admission envelope embeds a collection or body set that grows with a subtree, and preparation uses O(path depth + bounded page/run buffers) process memory without local scratch files or stored-file body reads. Readers encountering a pending transition use the pre-state before commit and the post-state after it. Root finalization and closed-gate garbage collection are idempotent consequences of durable state.
...
The append-only storage-schema ledger currently records five complete epochs: schema 001, the pre-aggregate format written by v0.1.0–v0.1.4; schema 002, the historical byte-aggregate intermediate; schema 003, the byte-and-file-count format written by v0.1.5–v0.1.14; schema 004, the untagged indexed metadata-only intermediate; and schema 005, the resumable-operation-preparation format first written by v0.2.0. It contains only adjacent transforms `001 -> 002`, `002 -> 003`, `003 -> 004`, and `004 -> 005`. Startup always executes the complete remaining suffix in that exact order. An unregistered, reordered, branching, direct-jump, or ambiguous epoch fails closed. Release validity ranges and schema transformations are separate: multiple releases can write identical epoch bytes, while an untagged intermediate epoch still remains a permanent migration entry point. Schema 001 predates the gate's `writerFeatures` field, so its exact historical gate representation is unbound even when its writer set carries non-ledger application features. Schemas 002 and later require the exact feature-bound gate representation declared by their epoch; an empty gate can identify only schema 001 and never a later epoch.

Each adjacent transform automatically closes and drains the canonical write gate under its own stable checkpoint ID. The aggregate transforms walk every user/area graph from the fixed root, reject missing children, cycles, multiple parents, unreachable directory roots, malformed records, byte/count overflow, and contradictions in already-persisted aggregates, and compute missing totals bottom-up from authoritative file entries and stored child aggregates. Schema 004 additionally obtains every file fingerprint through provider `Head`, builds the persistent directory, secondary-sort, descendant-content, and state indexes, directory content summaries, and duplicate catalog, and never opens a stored file body. Verification walks every content-index entry and resolves its relative file through the authoritative directory indexes before feature activation. New immutable IDs are deterministic functions of the source root identity and target epoch so concurrent migrators create identical prerequisites; each mutable root advances by native CAS. Historical page manifests require at most one legacy directory's entries while deriving several differently ordered schema-004 indexes; scope discovery, subtree traversal, current-index verification, and CAS-winner reconciliation remain streamed and do not retain a namespace or subtree. The `004 -> 005` edge is feature-only: schema 005 adds the durable preparing-operation representation without rewriting schema-004 application objects, so this edge closes and drains the gate, advances writer/superblock/gate feature binding, creates its own checkpoint, and deliberately performs no redundant directory graph walk.
...
Duplicate-review cleanup uses the version-pinned trash form. The request contains one to 100 `{path, version}` items and is mutually exclusive with the ordinary `paths` form. Each stale reviewed version fails with `precondition_failed` without moving the current path.
