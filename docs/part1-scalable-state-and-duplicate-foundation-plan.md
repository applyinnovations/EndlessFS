...
- Recursive copy/move catalog preparation still accumulates the complete subtree's prerequisites, copy list, and occurrences before it emits operation pages. This is now a Part 1 scalability item: make subtree preparation a durable paged operation phase with a restart cursor and O(depth + page-size) working memory before this PR is complete.
  - Append the next adjacent schema epoch for a `preparing` file-operation state; do not mutate schema 004 after its writer and migration boundary have been recorded.
  - Admit and persist the bounded operation header before traversal. Pin the source/destination manifests, gate version, request fingerprint, and deterministic traversal cursor so another replica can resume the same preparation.
  - Emit prerequisite references, server-side copy intents, occurrence changes, summary deltas, similarity changes, and ancestor content deltas into independently bounded hash-chained preparation runs. Runs are sorted and merged with fixed fan-in in provider storage; no local disk, object body, per-item journal, or subtree-sized map is allowed.
  - Build the final operation-step chain directly from those merged runs, conditionally seal the preparation, and only then enter the existing multi-root prepare/commit/finalize protocol. A changed pinned root fails before visibility preparation.
  - Closed-gate recovery resumes an admitted preparation from its last authenticated run/cursor; retention removes only terminal or provably unreachable preparation runs. Deterministic tests must pause at every run/page/seal boundary and enforce O(depth + page-size) buffers, bounded record bytes, and no repeated completed run after takeover.
