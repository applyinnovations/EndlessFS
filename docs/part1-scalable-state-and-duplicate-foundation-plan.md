...
- Ignore decisions are durable, owner-scoped policy records keyed by stable duplicate identities. Exact-group policy and directory-pair policy are distinct, so intentionally mirrored folders can be hidden without suppressing every occurrence of the same content. Moving or renaming a directory does not accidentally unignore it.
...
- Aggregate migration originally retained complete discovered/root maps, revisited every completed leaf after restart, and materialized a directory's complete descendant-content run while constructing schema-004 indexes. Part 1 now uses bounded provider listing, exact persisted transform/verify directory marks, and a fixed-fanout streaming content-index builder fed by a bounded merge of already-migrated child indexes. Completed immutable nodes are persisted as they fill and deterministically reused after interruption. Active traversal no longer retains a selected subtree: working memory is O(depth + one legacy directory + the bounded child-merge frontier).
...
- Recursive copy/move and non-empty directory replacement originally accumulated a complete subtree's prerequisites, copy list, occurrences, and catalog deltas before emitting operation pages. Part 1 appends schema epoch 005 with a durable `preparing` file-operation state. It admits a bounded operation header before traversal, pins source/destination roots and request identity, emits independently bounded authenticated preparation runs, reduces them through fixed-fan-in provider-backed merges, seals directly into operation-step pages, and then enters the existing multi-root prepare/commit/finalize protocol. Takeover resumes completed runs without rewriting them, changed roots fail before visibility preparation, and closed-gate retention removes only terminal or unreachable preparation artifacts. No local disk, stored-file body read, per-item journal, or subtree-sized process map is used.
- Whole-directory compatibility helpers remained compiled into the runtime after indexed mutation paths replaced them. Part 1 confines snapshot reconstruction and full-directory update helpers to test-only files. Runtime lookup, mutation, mismatch classification, clone, catalog traversal, and migration winner verification now use metadata roots plus bounded iterators.
- Duplicate-catalog migration rebuilt a complete owner/root inventory in maps before walking it. Part 1 streams owner scopes in provider order, retains only the active ancestor stack and one index page, and emits catalog effects incrementally.
- Migration scope discovery retained every owner scope and sorted the full set before transformation. Part 1 processes provider-ordered scopes one at a time and overflow-checks the streamed root count. The historical `003 -> 004` converter still retains one bounded-by-legacy-manifest directory because one old page set must be transformed into multiple differently ordered persistent indexes; it never retains a subtree or storage-set inventory. Current-schema verification and CAS-winner reconciliation are page-bounded and also validate the pinned descendant-content index.
- Automatic overlap-candidate discovery was not supplied by exact directory digests alone; requiring the Part 2 UI to scan the bucket or materialize an O(directory-pairs) matrix would violate the scalability goal. The bounded sketch/posting solution below closes this backend gap.
- Automatic overlap discovery now uses a 16-position deletion-safe MinHash summary embedded in each descendant-content tree. Inverted postings contain only stable discovery fields, so duplicate multiplicity and ordinary content changes rewrite no posting unless a slot minimum actually changes. Candidate enumeration reads the 16 matching posting ranges, canonicalizes repeats by first shared slot, and computes exact intersection/difference only for emitted pairs; it never persists a directory-pair matrix.
