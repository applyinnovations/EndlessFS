...
Grid images use explicit dimensions, asynchronous decoding, low fetch priority, and lazy requests. The browser keeps capability values and object URLs in memory only, aborts requests for evicted tiles, releases object URLs when tiles or viewers close, and discards any `ready` cache state that depended on a revoked object URL so a re-entering tile reacquires an exact-generation capability.
