...
| Artifact identity, validation, immutable generations, exact capabilities, expiry, corruption denial, runtime removal, and revalidation | reusable preview-store contract via `TestContractMemoryPreviewStore`, `TestContractDurablePreviewStoreOverObjectBackend`, and `TestContractDurablePreviewStoreOverGCSProtocolFake`; durable boundary, corruption, lifecycle, and startup-validation matrices |
