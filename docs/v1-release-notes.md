...
The follow-up embedded `/duplicates` workspace turns that bounded catalog into a review-first owner workflow. Exact folder and file groups load independently, reveal bounded locations, require one live copy to be kept, and move only the remaining loaded copies to recoverable Trash. Selected-folder overlap results distinguish exact, contained, and partial relationships using exact intersection and both differences; the browser previews one pinned 100-file cleanup page from either side and explicitly preserves both unique sets. Exact-group and stable pair ignores live in separate collapsed sections. URL state carries only an allowlisted duplicate kind and selected virtual folder path. The development fixture includes an exact project backup, a partial photography backup, and an intentional ignored published mirror; browser proof covers desktop and 320 CSS-pixel layouts without reading object bytes or summing overlapping savings.
