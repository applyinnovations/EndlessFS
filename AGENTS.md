...
- Publish directory/file changes only through CAS-controlled roots or a committed durable operation. Schema-006 direct browser uploads target newly allocated create-only final blob keys that remain unreachable until publication; legacy uploads and other intermediate provider results target immutable operation staging. Neither is visible file state before commit.
