...
func (e *Engine) withAdmission(ctx context.Context, intent string, action func() error) error {
...
func (e *Engine) admit(ctx context.Context, intent string) (admissionLease, error) {
	...
	candidate := storageformat.Admission{
		...
		CreatedAt: now, ExpiresAt: now.Add(e.leaseTTL), IntentDigest: storageformat.Digest([]byte(intent)),
	}
	...
	if secondGate.Mode != storageformat.GateOpen || secondGate.Epoch != gate.Epoch || secondEnvelope.LogicalVersion != gateEnvelope.LogicalVersion || gateObject.Version == "" {
	...
	return admissionLease{key: key, version: admittedVersion}, nil
}
