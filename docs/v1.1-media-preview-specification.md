...
`contentID` and `contentVersion` are provider-independent domain values in canonical directory records and never appear in public HTTP responses. Upload completion to a new path assigns them. Rename, move, directory-tree move, trash, and restore preserve them. Copy assigns a new `contentID`. Replacing file bytes preserves the existing `contentID` and always assigns a new `contentVersion` and `contentModifiedAt`. Delete followed by a later new upload assigns a new `contentID`. Thin object-store adapters MUST NOT derive, interpret, or persist these values in provider custom metadata. Client-supplied values are never accepted.

Canonical file records created before v1.1 require no preprocessing. The portable engine deterministically derives an upgrade identity from the canonical blob identity and rendering-relevant canonical fields, never from a provider generation, ETag, timestamp, bucket, or metadata. It materializes that identity in the canonical directory record on the next content-identity-relevant mutation. Reads remain side-effect free; rename, move, trash, restore, replacement, replica takeover, and raw-copy cutover retain the same semantics as newly created records.
