...
type ObjectBackend interface {
    ...
    Open(context.Context, ObjectKey, OpenObjectRequest) (VersionPinnedObjectReader, error)
...
`Get` is for bounded canonical control records. `Open` returns a version-pinned streaming reader and is mandatory for file bodies; hashing or verifying a file, including a provider-maximum multi-terabyte object, MUST use bounded process memory and MUST NOT materialize the body in a byte slice.
...
The append-only storage-schema ledger currently records three complete epochs: schema 001, the pre-aggregate format written by v0.1.0–v0.1.4; schema 002, the historical byte-aggregate intermediate; and schema 003, the byte-and-file-count format written from v0.1.5. It contains only adjacent transforms `001 -> 002` and `002 -> 003`. Starting at schema 001 therefore executes both transforms in order; starting at schema 002 executes only the second; starting at schema 003 executes none. An unregistered, reordered, branching, or ambiguous epoch fails closed. Release validity ranges and schema transformations are separate: multiple releases can write identical epoch bytes, while an untagged intermediate epoch still remains a permanent migration entry point. Schema 001 predates the gate's `writerFeatures` field, so its exact historical gate representation is unbound even when its writer set carries non-ledger application features. Schemas 002 and later require the exact feature-bound gate representation declared by their epoch; an empty gate can identify only schema 001 and never a later epoch.

Each adjacent transform automatically closes and drains the canonical write gate under its own stable checkpoint ID. The aggregate transforms walk every user/area graph from the fixed root, reject missing children, cycles, multiple parents, unreachable directory roots, malformed records, byte/count overflow, and contradictions in already-persisted aggregates, and compute missing totals bottom-up from authoritative file entries and stored child aggregates. New page and manifest IDs are deterministic functions of the source root identity and target epoch so concurrent migrators create identical immutable prerequisites; each root advances by native CAS. An interrupted transform may contain a reviewed safe mixture of its source, target, and later already-published records; the same edge resumes and verifies it without downgrading later state. After a complete second verification, startup conditionally updates the writer set and superblock, binds the target feature set into the closed gate, creates or verifies that edge's checkpoint, and reopens at a new gate epoch before continuing to the next edge.

Checkpoint construction journals each completed key, size, and SHA-256 body digest as an authenticated provider-independent work record under that checkpoint's excluded namespace. It streams each version-pinned body through a fixed-memory SHA-256/CRC32C calculation. A restart resolves the deterministic work key for each listed authoritative object directly, validates the work record against the same closed gate, canonical key, role, and current listed size, and resumes without loading the complete journal into memory. It never persists or trusts a provider generation, ETag, checksum, or other native identifier.

New checkpoints use the `checkpoint-v2` representation. Its constant-size root contains storage/writer/gate identity, state/file object counts, inventory-page count, and the final inventory-chain digest. The inventory is encoded in independently bounded `checkpoint-inventory-page-v1` records of at most 512 entries. Pages have deterministic monotonically numbered keys, bind the checkpoint ID and gate epoch, carry the preceding page digest, and contain entries in canonical checkpoint-work-key order; the root binds the final page digest. Page count grows without extending any existing page or the root. Construction, enumeration, raw-copy, and verification stream pages and objects, so process memory is independent of total object count and individual file size. A destination independently streams and hashes every body, requires exact state/file counts, rejects missing, extra, misplaced, reordered, malformed, or chain-inconsistent pages or objects, and never relies on provider listing order as durable state. Existing `checkpoint-v1` single-record checkpoints remain readable and verifiable. Checkpoint representation is versioned independently from the authoritative storage-schema ledger because it neither changes nor rewrites an authoritative application object; a historical migration edge may emit the current checkpoint representation while preserving its immutable authoritative transformation.

A checkpoint created from the complete journal is compared directly with those journaled entries before the same process opens the still-closed gate, avoiding an immediately duplicated full-object scan; raw-copy destination verification continues to read and hash the complete destination independently. Every boundary is idempotent and crash-resumable. Gate feature binding prevents an already-running predecessor binary from admitting a post-migration mutation. Active work can delay startup; no timeout forces it. Arbitrary provider objects outside the canonical graph are never inferred or imported.
...
The `admissions`, `staging`, `leases`, and per-checkpoint `work` namespaces are transient and are excluded from portable logical state and checkpoint inventories. Admissions, leases, and checkpoint work live in the state backend; staging lives in the file backend. Inventory pages under a completed checkpoint's `inventory` namespace are durable checkpoint metadata: they are excluded from the authoritative application inventory but MUST be copied and verified with the checkpoint root. Admissions contain only portable writer-gate tickets linked to durable operation records. Staging contains only immutable operation-specific data that is unreachable from committed directory manifests. Leases may contain only authenticated-encrypted, bounded backend-native resumable-session, multipart/block-upload, or rewrite/copy continuation data. Lease bodies MUST contain the backend kind and expiry and MUST NOT be required to reopen durable logical state. Checkpoint work contains only the checkpoint ID, closed gate epoch, provider-independent backend role, canonical object key, body size, SHA-256 body digest, application-computed CRC32C integrity assertion, and an application-keyed authentication proof; it contains no native provider value and is inert outside its exact closed-gate checkpoint. Resumption verifies the expected CRC32C and size through the backend integrity contract before trusting the authenticated SHA-256 journal entry. A checkpoint requires no admitted ticket, live staging operation, or live lease; cancelled/expired tickets, completed checkpoint work, and unreachable staging garbage may be removed after their coordinating operation and are never copied as authoritative state.
...
4. Write a constant-size canonical checkpoint root in the state backend containing the storage-set ID, writer-set and gate epoch, format/protocol versions, logical checkpoint revision, role counts, page count, and final inventory-chain digest. Write its bounded hash-chained inventory pages containing the canonical role, object key, size, and SHA-256 body digest for every authoritative object. The checkpoint includes the closed gate and writer-set record, but excludes its root/pages and the transient admission, staging, lease, and work namespaces from the authoritative application inventory.
