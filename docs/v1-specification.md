...
In a real deployment, the configured state bucket and file bucket form the authoritative storage set. They MAY be the same bucket. The state bucket holds the small amount of EndlessFS state and filesystem metadata; the file bucket holds immutable user file blobs and decode-only legacy upload staging. The application container is replaceable and has no required persistent filesystem. Every authoritative object uses the canonical format and placement rules in section 9; no backend-specific database, index, third persistence role, or application filesystem may be required to reopen it.
...
- Records use versioned JSON with strict decoding, size limits, and unknown-field rejection. A record limit is a per-shard denial-of-service boundary, never a total-file-count or total-storage limit: no canonical record may contain a collection that grows with the complete storage set. Growing collections use independently bounded immutable pages or records behind a constant-size root.
...
type ObjectBackend interface {
    Head(context.Context, ObjectKey) (ObjectInfo, error)
...
`Get` is for bounded canonical control records and MUST NOT be used with blob or upload-staging keys. Ordinary file-control code receives only a body-free metadata interface (`Head`, `Verify`, and `List`) plus conditional server-side `Copy`/`Delete` and direct-transfer capability methods. A version-pinned body reader is a separate optional-feature capability. The initial exemption registry contains only image preview generation; every future exemption requires an explicit named feature, source-level policy update, and positive/denial tests. Hashing, inventory, duplicate detection, checkpointing, migration, and reconciliation MUST NOT download or stream stored file bodies through the Go service.
...
To mutate schema-009-or-later same-domain authority, the portable engine MUST read the canonical consistency-domain head together with its request-local backend-native version, validate logical preconditions against the authenticated head and referenced immutable pages or page pack, prepare changed immutable metadata, and conditionally publish the next head against that native version. It then discards the native version. The committed delta or compacted outcome binds the complete normalized intent fingerprint and result. A retry reads canonical authority and returns that result; it never assumes native identifiers survive.

Schema 011 MAY encode the pages required by one bounded domain snapshot in one deterministic content-addressed pack. A pack MUST be no larger than 4 MiB compressed or 32 MiB after bounded expansion; MUST bind the domain ID, domain kind, pack ID, sorted page digests, and canonical page bodies; and MUST authenticate every page exactly as the unpacked representation does. The pack ID MUST bind the complete mutation identity before any page is written, so two mutations sharing an early page cannot alias divergent later pages. Missing, oversized, non-canonical, corrupt, misbound, or digest-inconsistent packs fail closed. Domains that do not fit the pack envelope retain immutable copy-on-write pages. Encoding choice never changes the one-head-CAS visibility, replay, fencing, checkpoint, or raw-copy contract.

The domain revision and captured native head condition fence an ordinary schema-009-or-later writer. A visible same-domain commit increments the contiguous portable domain revision and uses the current request-local native condition on the same head object. Mutation, transition preparation/finalization, or freeze changes that head before a paused writer can publish, so the paused writer's conditional commit fails. Historical portable operation fences remain decode-only migration input and do not justify a runtime lease or admission object around an ordinary mutation.
...
A schema-011 packed head names one immutable pack containing the authenticated
page records needed by that bounded snapshot. One request-local decode cache
serves every lookup in the operation. A bounded materialized mutation writes
one successor pack and one conditional head; it MUST NOT retain one provider
object or one outcome row per selected logical item. Uniform batch results are
reconstructed from the fingerprint-bound compact transaction descriptor.
...
owner head CAS. The ordinary batch limit is 10,000 selected roots. A directory
edge retains its immutable child root, so provider requests and file-backend
work are independent of descendant count.

### Historical schemas 001-009 (migration input only)
...
fall back to these key families. Schema 010 retains the schema-009 ordinary
runtime representation and adds an authority-conservation feature and migration
activation invariant. Schema 011 retains that authority, accepts authenticated
schema-010 page roots after migration, and writes the bounded packed form on a
subsequent eligible mutation. Schema 009 and schema 010 are therefore migration
entry points, not alternate writer protocols after schema 011 is current.
...
File-count scale is partitioned by owner, area, and directory; there is no storage-set-wide directory manifest, file table, or aggregate record. Adding users, directories, or files adds bounded canonical objects within those partitions rather than extending one global state document. The only complete-storage-set structure is the paged checkpoint inventory defined below, whose root remains constant size.

A single-directory content change writes only changed immutable search-tree nodes plus a new manifest and conditionally replaces `directory.json`; that root replacement is its visibility point. Directory mutations apply entry deltas to recursive counts and the content accumulator and never reconstruct an unchanged directory. Unreferenced nodes or manifests are garbage, never visible entries. Schema-005 preserves the bounded durable `preparing` representation for recovery of historical recursive work. Schema-006 copy, move, trash, restore, permanent delete, and non-empty replacement pin or detach one immutable subtree snapshot and rewrite only the namespace paths and ancestor manifests needed to reach it. Their provider-call and record counts are independent of descendant count, no file-backend copy/delete is issued, and live/trash placement is independent of the immutable snapshot's physical metadata area. Schema-007 reduces an ordinary plan of at most 64 preparation items in bounded process memory before admission; if the reduced canonical operation does not fit, it uses the schema-005 resumable external preparation path. The sealed operation CAS-locks only the affected namespace visibility roots in canonical order and conditionally changes its durable operation record to `committed`. Large-plan prerequisite bodies live once in immutable operation staging and are promoted with digest verification. A schema-007 inline operation retains only sorted key/digest references after its transient admission has published the immutable prerequisites, never their bodies. No operation or admission envelope embeds a collection or body set that grows with a subtree, and preparation uses O(path depth + bounded page buffers) process memory without local scratch files or stored-file body reads. Readers encountering a pending transition use the pre-state before commit and the post-state after it. Root finalization and closed-gate garbage collection are idempotent consequences of durable state.
...
File blobs are immutable and live only under the file backend's blob namespace for their owner. Schema-006 direct uploads allocate that final blob ID before issuing the provider capability; successful completion verifies its provider-attested metadata and publishes the reference without a staging-to-blob rewrite. An aborted or expired unpublished blob stays unreachable and is collectible. File-entry records in state-backend directory pages contain the blob ID, normalized name, size, safe media type, integrity digests, timestamps, and portable logical version. A logical file contributes one to the recursive file count irrespective of its byte length. Directory entries use `size` and `fileCount` for the child's recursive byte and file-count aggregates; directories themselves do not contribute to `fileCount`. Every directory root's byte aggregate MUST equal the overflow-checked sum of child sizes, its file-count aggregate MUST equal the overflow-checked sum of one per immediate file plus every immediate child directory's count, and every parent directory entry aggregate MUST equal the referenced child snapshot aggregates. Upload and replacement update affected ancestors through the committed visibility operation. A subtree namespace mutation reuses its already-verified aggregate values and rewrites only the ancestors on the changed namespace paths. A zero-byte file therefore has `size: 0` and `fileCount: 1`; an empty directory has zero for both. Negative, overflowing, missing, or inconsistent aggregates fail closed. Same-owner copy is a logical reflink to the existing immutable blob or directory snapshot and MUST NOT issue a provider copy; cross-user blob references and cross-user deduplication are forbidden.

The duplicate catalog is owner-scoped derived canonical state. A confirmed file group ID is derived from the exact `(size, MD5, CRC32C)` tuple; a missing checksum produces no confirmed group. A directory group ID is derived from recursive bytes, recursive file count, and the structural content digest, which includes every relative name, kind, nested digest, and file tuple. Directory equality is therefore exact tree equality, not merely equal aggregate totals. Each manifest's content view contains one descendant-file occurrence with group, relative path, and size. Logical versions are deliberately resolved from the authoritative file entry only for the bounded set placed into a reconciliation plan, so a same-content replacement structurally shares the content index. Catalog occurrence roots bind group, kind, live/trash area, virtual path, logical version, bytes, and file count. Bounded summary shards maintain occurrence counts without a storage-set-wide mutable counter. Each non-empty user-addressable directory also has one small posting per MinHash position, keyed by `(position, minimum, area, directory ID)` and containing only the stable virtual-path locator needed to resolve the live manifest. Live and trash area roots are navigation containers and never duplicate-reconciliation candidates. Schema-007 readers ignore immutable schema-006 root postings and new writes do not mutate them. A file mutation rewrites a posting only if that position's minimum changes; adding another occurrence of an existing group rewrites none. Local file/directory mutations prepare catalog, changed postings, and content-index changes in the same fenced operation as the visible directory transition. Schema-006 and later subtree aliases publish only their new exact-directory root occurrence and reuse the pinned descendant-content view; they do not manufacture per-descendant file occurrences or claim reclaimable physical bytes for reflink aliases.
...
The append-only storage-schema ledger records eleven complete epochs: schema 001, the pre-aggregate format written by v0.1.0–v0.1.4; schema 002, the historical byte-aggregate intermediate; schema 003, the byte-and-file-count format written by v0.1.5–v0.1.14; schema 004, the untagged indexed metadata-only intermediate; schema 005, the resumable-operation-preparation format written by v0.2.x; schema 006, the persistent namespace-snapshot and direct-final-upload format first written by v0.3.0; schema 007, the untagged bounded-inline-operation and user-addressable duplicate-directory format; schema 008, the untagged consistency-domain and owner-namespace-graph format; schema 009, the typed invariant-aligned state-domain and helpable cross-domain-transition format first written by v0.4.0; schema 010, the authority-conservation format first written by v0.5.0; and schema 011, the packed-domain and bounded-upload-transaction format first written by v0.7.0. It contains only adjacent transforms `001 -> 002`, `002 -> 003`, `003 -> 004`, `004 -> 005`, `005 -> 006`, `006 -> 007`, `007 -> 008`, `008 -> 009`, `009 -> 010`, and `010 -> 011`. Startup always executes the complete remaining suffix in that exact order. An unregistered, reordered, branching, direct-jump, or ambiguous epoch fails closed. Release validity ranges and schema transformations are separate: multiple releases can write identical epoch bytes, while an untagged intermediate epoch still remains a permanent migration entry point. Schema 001 predates the gate's `writerFeatures` field, so its exact historical gate representation is unbound even when its writer set carries non-ledger application features. Schemas 002 and later require the exact feature-bound gate representation declared by their epoch; an empty gate can identify only schema 001 and never a later epoch.
...
Each adjacent transform automatically closes and drains the canonical write gate under its own stable checkpoint ID. The aggregate transforms walk every user/area graph from the fixed root, reject missing children, cycles, multiple parents, unreachable directory roots, malformed records, byte/count overflow, and contradictions in already-persisted aggregates, and compute missing totals bottom-up from authoritative file entries and stored child aggregates. Schema 004 additionally obtains every file fingerprint through provider `Head`, builds the historical indexes and duplicate catalog, and never opens a stored file body. The `004 -> 005`, `005 -> 006`, and `006 -> 007` edges add their historical preparation, snapshot, and bounded-plan representations. The `007 -> 008` edge deterministically stages routed state, terminal operations/uploads, Trash metadata, and the owner namespace graph into consistency domains; installs content-addressed pages; freezes installed domains; creates its checkpoint; advances writer/superblock/gate feature binding; reopens; and idempotently unfreezes. The `008 -> 009` edge authenticates every predecessor domain and key, wraps each unchanged application payload in its exact typed state record, deterministically repartitions authority by invariant into namespace, owner-identity, owner-jobs, administration, and capability domains, installs and freezes the complete target catalog, checkpoints it, advances the `transactional-state-domains-v1` feature binding, reopens, and unfreezes. The `009 -> 010` edge walks the actual retained schema-007 state-index roots and their referenced immutable state-version bodies; writes durable receipts that bind every exact source root/key/version/body to its typed target domain/key/type/value; installs only missing authority; and independently re-walks every source, receipt, target and catalog binding before activation. An unequal target, missing or changed source, malformed index, or forged, missing or corrupt receipt fails closed. These edges reference every immutable blob in place and perform no file body read or provider copy. Every edge is deterministic, resumable, multi-replica safe, and advances only to its adjacent successor.

Schema 010 and every later epoch MUST register a pre-activation authority verifier in its immutable ledger edge. The central writer-set activation path MUST execute that verifier while the gate and target domains remain frozen and MUST reject an absent or failing verifier. An edge implementation returning success or writing target feature markers is insufficient. For schema 010, the verifier independently reloads the durable conservation proof and proves the complete source-to-receipt-to-target relation. Future edges MUST provide an equivalently complete proof for every authoritative family they can change; a sampled count, successful decode, or engine-open check is not a conservation proof.

Schema 011 changes no predecessor application value. Its verifier MUST
nevertheless authenticate the complete current typed-domain catalog, heads,
pages, namespace closure, and checkpoint binding while its own gate is closed;
feature markers alone cannot activate the epoch.

The fixture matrix distinguishes writer-profile fixtures from complete semantic corpora. For every released predecessor profile, immutable bytes produced by that exact predecessor MUST contain every application authority family it can write, together with an independent semantic oracle. The full suffix MUST preserve those exact values in single- and split-backend layouts, complete a real cryptographic passkey authentication where identity is enabled, and perform a new mutation. Migration qualification MUST run the owning migration packages in full and its 98% denominator MUST include every numbered production migration file plus the ledger; a permissive test-name or source-file regex that silently excludes a later edge is forbidden.

The `010 -> 011` edge is feature-only. It closes and drains the gate, independently verifies the complete typed-domain authority under the exact frozen checkpoint, advances the writer/superblock/gate feature signature, creates its own checkpoint, and reopens at a new epoch. It neither rewrites file blobs nor eagerly rewrites predecessor pages; authenticated schema-010 pages remain readable until an eligible mutation publishes their schema-011 packed successor. Its transient provider leases and upload progress are excluded from authoritative checkpoint closure.
...
New checkpoints use the metadata-only `checkpoint-v3` representation. Its constant-size root contains storage/writer/gate identity, state/file object counts, inventory-page count, and the final inventory-chain digest. The inventory is encoded in independently bounded pages of at most 512 entries. Every entry contains canonical role, object key, exact size, normalized provider-attested MD5, and normalized provider-attested CRC32C. Pages have deterministic monotonically numbered keys, bind the checkpoint ID and gate epoch, carry the preceding page digest, and contain entries in canonical key order; the root binds the final page digest. Page count grows without extending an existing page or the root.

Construction makes one ordered metadata traversal per backend role and emits pages directly with O(page-size) memory. Verification makes a separate ordered metadata traversal and compares the same metadata tuple. Neither source nor destination verification downloads, streams, or hashes authoritative bodies through EndlessFS. Missing or partial provider integrity metadata fails closed. The provider's metadata assertion is therefore part of the supported-backend contract; it is not durable provider identity and it cannot be replaced with an ETag. Checkpoint v1/v2 roots and their object-per-entry work journals are retired migration inputs: while the gate is closed they are removed or replaced by a complete v3 checkpoint, and they never authorize reopening. Checkpoint representation remains versioned independently from the authoritative storage-schema ledger because a checkpoint does not rewrite application objects.

Every boundary is deterministic, idempotent, and crash-resumable. Gate feature binding prevents an already-running predecessor binary from admitting a post-migration mutation. Active work can delay startup; no timeout forces it. Arbitrary provider objects outside the canonical graph are never inferred or imported.
...
Schema-009-or-later `leases`, upload-transaction progress, `maintenance`, migration-stage, state-query-snapshot,
...
Mutation outcomes and idempotency bindings remain replayable for 30 days; Trash retention is separate and indefinite until the owner permanently deletes it. Outcome expiry is itself authenticated by each domain's expiry tree. After a schema-011 checkpoint is published under the closed gate, its authenticated inventory is the immutable mark set for one checkpoint-bound garbage-collection session. Discovery writes a deterministic portable garbage plan with at most 128 entries per page and authenticates the complete plan before the first deletion. The session stores only its checkpoint/gate/inventory/plan binding and sweep position. It sweeps only explicitly recognized domain pages, packs, or inert heads, resolved transition records, rebuildable projections, transient leases/progress, state-query snapshots, and unreachable immutable blobs. It never opens a file body. Every deletion is conditional on the native version from the frozen discovery/listing result, and the closed gate is revalidated immediately before destructive work. Independent deletes use bounded concurrency; one durable cursor publication makes the page restartable. Unknown reserved objects are never deletion-eligible. Writes reopen only after the exact terminal session and checkpoint are reverified. Completed predecessor GC sessions and marks remain excluded durable compatibility residue while a supported predecessor sweeper could still exist; deleting them earlier can make a lagging sweeper unsafe. Forged or stale sessions, a changed gate/checkpoint binding, corrupt/cyclic/missing plan pages, contradictory aggregates, ambiguous reachability, unordered entries, changed native versions, or misplaced objects fail closed.
...
For every schema-009-or-later same-domain mutation:
...
Schema-009-or-later public uploads target a newly allocated create-only final blob key.
...
Creating a schema-011 upload batch publishes all owner-scoped upload intents in
one packed namespace mutation, creates the unavoidable independent provider
sessions with at most 100 concurrent workers, and seals transient cumulative
lease progress at each 1,000-item boundary. Returning capabilities requires no
second authoritative activation mutation and MUST NOT run one state transaction
per item.
...
- Every complete durable data model belongs to one immutable epoch in the append-only production storage-schema ledger. A future key, body, semantic invariant, or required-feature change appends the next epoch and exactly one adjacent `N -> N+1` transformation. Existing epoch IDs, ordering, validity ranges, feature signatures, checkpoint identities, and transforms cannot be inserted, reordered, branched, bypassed, or rewritten. Silent in-place reinterpretation and bespoke release-specific startup branches are forbidden.

Every epoch MUST have immutable raw state/file-backend fixtures produced by code that actually wrote that epoch and bound to a source tag or commit plus a hard-coded SHA-256 digest. This includes untagged intermediate epochs and every durable application feature profile that a release could write, including preview-disabled and configured preview-store profiles. A predecessor binary that can durably change state before failing or being interrupted MUST also contribute an immutable intermediate-state fixture at that exact boundary. Release-to-epoch validity ranges are recorded separately in the production ledger; multiple releases MAY share an epoch only when their durable bytes and semantics are identical. Current-code reconstructions, hand-edited approximations, decoder-only tests, current-code-only crash simulations, and already-migrated fixtures are not sufficient migration evidence.

A current binary MUST import every epoch/profile and predecessor-intermediate fixture into fresh single- and split-backend stores, resolve and execute the entire remaining ordered ledger suffix, verify the full authoritative result, and successfully perform a post-upgrade mutation. The application test suite MUST also derive each supported durable writer profile through the production startup configuration builder and open its matching historical fixture, so a feature-minimal engine cannot stand in for the released application. Every adjacent edge MUST prove deterministic restart at each durable boundary and two-to-eight-replica convergence across those profiles; the composed oldest-to-current path MUST prove exact edge order. Corrupt, truncated, mixed, newer, unreachable, or semantically inconsistent state fails closed. Transformations are forward-only, gate-quiesced, fenced, deterministic, idempotent, crash-resumable, and provider-independent; ordinary runtime decoders remain strict.

Every pull-request gate MUST execute this complete epoch matrix and enforce at least 98% aggregate statement coverage across the production migration ledger and migration implementation. Coverage is only a backstop: every durable boundary, denial path, restart point, and concurrent CAS outcome still requires an explicit behavioral assertion, including deterministic transport interruption at every object operation. Release CI MUST reject a candidate tag outside all declared release validity ranges or mapped to an epoch without an immutable fixture before building or publishing artifacts. A durable schema change MUST add the failing-before-green fixture, new epoch, adjacent transform, complete-suffix tests, crash/concurrency/denial coverage, and matching specification evidence in the same change. Removing support for any epoch, transform, fixture, or supported release range requires an explicit major-version compatibility policy and reviewed specification change.
...
- The default page size is 200 entries; the maximum is 10,000.
...
- A single upload and a batch of up to 10,000 upload initializations are supported.
- The browser coalesces concurrently queued upload initialization into batches
  of at most 10,000 before starting their independently concurrent direct data
...
- The UI uses the 100-worker shared file queue implemented by Google's
  ...
  bound for smaller selections. The same 100-worker bound applies to independent
  provider session initiation, metadata verification, and cancellation inside
  a 10,000-item server transaction. Explicit browser Data Saver intent reduces it to
...
- Batch completion and cancellation persist an authenticated prefix every 1,000
  items, so process loss repeats at most the current 1,000-item segment. The
...
- Before a selected file or completed folder discovery begins, the browser asks
  ...
  planning. The other strategies submit at most 10,000 `(transfer ID, virtual
...
- Batch requests contain at most 10,000 selected source items and return per-item results plus an overall operation ID.
...
|---|---|---|
| GET | `/healthz` | Process liveness only; bound before storage migration or other durable runtime assembly and contains no provider details or secrets. |
| GET | `/readyz` | Valid configuration, completed storage migration, and reachable configured provider; returns unavailable until complete runtime assembly. |
...
| POST | `/api/v1/uploads/batch` | Create up to 10,000 destination-bound upload capabilities with safe batch recovery bindings. |
| POST | `/api/v1/uploads/batch/complete` | Verify and atomically publish up to 10,000 uploaded objects with bounded restart progress. |
| DELETE | `/api/v1/uploads/batch` | Abort up to 10,000 active sessions; a proven complete batch may use its compact abort overlay. |
...
| `ENDLESSFS_GCS_FILE_BUCKET` | Required with `gcs` | Private GCS file bucket used for immutable blobs, unpublished direct-final uploads, and decode-only legacy upload staging; also used for state when no distinct state bucket is configured. Excluded from canonical keys and bodies. |
...
`TestProviderBudgetProductionScaleScenariosConformToTargets` and
`TestProviderBudgetTargetPayloadEnvelopesAreMeasured`. The schema-010 figures
in that record are explicit nonconforming historical baselines. Schema 011 is
provider-economics qualified only when its real traced workloads meet or improve
every corresponding target and the exact observations are appended to the
ratchet ledger. A target tightens whenever implementation proves a cheaper safe
request shape. Any revision requires measured payload/provider evidence and a
demonstrated correctness invariant; implementation convenience is never
evidence.
...
layout, packed-page mapping, blob layout, retained-outcome/idempotency layout,
upload lease/progress/abort mapping, projection layout, garbage-collection
plan/session, and checkpoint format.
...
**AC-085** — Schema-009-or-later direct uploads target only a newly allocated
...
and never expose a partial or corrupt entry. A schema-011 batch publishes up to
10,000 intents with one bounded owner-head mutation, initiates independent
provider sessions with 100-worker concurrency, persists authenticated 1,000-item
restart boundaries, and completes or cancels through one final owner-head
transition; historical staging remains decode-only migration compatibility.
...
**AC-092** — A schema-011 checkpoint-bound collector reclaims only recognized
objects absent from the authenticated inventory, authenticates a complete
portable 128-entry-page garbage plan before deleting, uses bounded concurrent
conditional deletion and a resumable cursor without reading file bodies,
denies unknown, changed, corrupt, or misbound state, converges under crashes and
concurrent replicas, and reaches a verified terminal session before the gate
reopens.
...
- [x] Directory children use immutable high-fan-out trees behind one owner
  ...
  post-commit views.
- [x] Schema-009-or-later browser uploads target a newly allocated create-only final
...
- [x] Schema-011 upload batches publish up to 10,000 owner-scoped intents in one
  bounded pack while independent provider sessions use 100-worker concurrency;
  completion/cancellation restart at authenticated 1,000-item boundaries and
  the shared provider contract proves replay and failed-batch residue denial.
...
| `CreateUpload` | V4 signed `PUT` or server-initiated resumable session bound to one newly allocated immutable final blob key; legacy schema-001 records may retain an immutable staging key |
