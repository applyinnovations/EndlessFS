...
| Bounded inline ordinary-operation plans with prerequisite references, automatic resumable-page fallback, bounded provider-backed run reduction, immutable multi-root preparation, one commit point, takeover fencing, and crash recovery | `TestProviderBudgetTrashAndRestore`, `TestDirectOperationFallsBackToResumablePreparationWhenInlinePlanIsTooLarge`, `TestOperationPreparationRunsMergeWithBoundedMemoryAndIdempotentReplay`, `TestResumedOperationPreparationDoesNotRewriteCompletedRuns`, `TestAdmittedOperationHeaderResumesWithoutProcessLocalPreparationState`, `TestCreateDirectoryReplacementAdmitsBeforeSubtreePreparation`, `TestReplicaDropAfterRootPrepareRecoversAtOneCommitPoint`, `TestSupersededReplicaCannotCommitWithTakeoverFence`, `TestReplicaDropAfterCommitOrFinalizationRecoversPostCommitView` |
| Schema-006 persistent namespace snapshots make folder Trash/restore cost independent of descendants; logical copy and direct-final upload publication perform no file-provider copy | `TestNamespaceFolderMoveCostDoesNotScaleWithDescendants`, `TestLogicalCopyAndUploadPublicationUseNoFileBackendCopy`, provider contract `recursive operations idempotency and faults` |
...
| Incremental exact duplicate groups, lazy descendant-content views, overlap candidates, stable exact/pair ignore policy, bounded reconciliation, same-content structural sharing, posting write locality, and closed-gate tombstone denial/cleanup | `TestDuplicateCatalogTracksFileAndExactDirectoryGroupsIncrementally`, `TestDuplicateCatalogFollowsSubtreeRootsWithoutWalkingDescendants`, `TestDuplicateCatalogUsesBoundedAuthenticatedPagination`, `TestDuplicateReconciliationPagesPersistentDirectoryContentIndexes`, `TestDirectoryContentIndexSharesRootAcrossSameContentReplacement`, `TestDuplicateSimilarityPostingsDoNotRewriteWhenContentSetIsUnchanged`, `TestClosedGateMaintenancePrunesDuplicateTombstonesAndRetainsPreferences`, `TestClosedGateMaintenanceRejectsUnresolvedDuplicateTransitions` |
...
| Seven-epoch append-only storage-schema ledger, release validity ranges, immutable fixture for every epoch/profile (including untagged schemas 002/004/007 and released schemas 005/006), complete ordered suffix from every entry point, feature-only `004 -> 005`, `005 -> 006`, and `006 -> 007`, old-writer fencing, split-backend support, every-edge crash resumption, metadata-only open-ended checkpoint inventories, concurrent starters, upload drain, and corrupt-graph/page/index denial | `TestStorageSchemaLedgerIsLinearAndAppendOnly`, `TestStorageSchemaLedgerDefinesReleaseValidity`, `TestMigrationEveryRegisteredStorageSchemaOpensAndMutatesWithCurrentCode`, `TestEveryHistoricalReleaseMapsToRegisteredStorageSchemaFixture`, `TestMigrationEveryLedgerEdgeResumesAfterEveryDurableBoundary`, `TestMigrationOldestSchemaTraversesLedgerEdgesInOrder`, `TestMigrationCheckpointInventoryResumesFromBoundedPagesWithoutReadingFileBodies`, `TestMigrationCheckpointInventoryRejectsForgedRestartPage`, `TestMigrationDoesNotReadFileBodiesAgainImmediatelyBeforeOpeningWrites`, `TestCheckpointSupportsInventoryBeyondCanonicalRecordLimit`, `TestCheckpointV1RemainsReadable`, `TestCheckpointVerifierRejectsInvalidBootstrapAndCheckpointRecords`, `TestMigrationReportsStageAndProviderIndependentInventoryProgress`, `TestEightReplicasConcurrentlyMigrateSchema001Fixture`, `TestEightReplicasConcurrentlyMigrateSchema002`, `TestSchema001MigrationResumesAfterUploadRecordUpgrade`, `TestSchema001MigrationWaitsForAndDrainsActiveUpload`, `TestSchema001MigrationRejectsCorruptTrees`, `TestSchema002MigrationRejectsInconsistentPersistedAggregate`, `TestMigrationWinnerValidationRejectsEveryInconsistentResult` |
