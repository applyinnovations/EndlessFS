...
| Fail-closed GCS wire classification, exhaustive contract/HTTP-route cataloguing, exact append-only request-count/modeled-price/modeled-latency ratchets, and composed production-scale evidence for state, file, direct data-plane, preview, and client workflows | `TestGCSEconomicsProtocolCoverage`, `TestGCSTransferEconomicsProtocolCoverage`, `TestProviderBudgetCatalogCoversApplicationContractsAndRatchets`, `TestProviderBudgetCatalogClassifiesEveryRegisteredHTTPRoute`, `TestProviderBudgetProductionScaleScenarios`, `TestProviderBudgetStateStoreContract`, `TestProviderBudgetTrashAndRestore`, `TestProviderBudgetDurablePreviewStore`, `TestRatchetLedgerOnlyTightensAndCannotDropPathways`; `docs/provider-request-surface-audit-2026-09-01.md`; `nix run .#test-provider-budget` |
