...
func TestLoadReadsValidatedProcessEnvironment(t *testing.T) {
	keys := []string{
		...
		"ENDLESSFS_GCS_BUCKET", "ENDLESSFS_GCS_STATE_BUCKET", "ENDLESSFS_GCS_SIGNING_SERVICE_ACCOUNT", "ENDLESSFS_WRITER_SET_ID",
...
func TestParseGCSUsesOnlyBucketAndPortableWriterConfiguration(t *testing.T) {
	...
	if cfg.StorageProvider != "gcs" || cfg.GCSBucket != values["ENDLESSFS_GCS_BUCKET"] || cfg.GCSStateBucket != cfg.GCSBucket || cfg.GCSSigningAccount != values["ENDLESSFS_GCS_SIGNING_SERVICE_ACCOUNT"] || cfg.WriterSetID != values["ENDLESSFS_WRITER_SET_ID"] {
...
func TestParseGCSAllowsSeparateOrSharedStateBucket(t *testing.T) {
	...
	base := map[string]string{
		"ENDLESSFS_STORAGE_PROVIDER": "gcs",
		"ENDLESSFS_GCS_BUCKET":       "endlessfs-files",
		"ENDLESSFS_WRITER_SET_ID":    "EREREREREREREREREREREQ",
	}
	for name, stateBucket := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			if cfg.GCSBucket != "endlessfs-files" || cfg.GCSStateBucket != stateBucket {
				t.Fatalf("GCS buckets = files %q, state %q", cfg.GCSBucket, cfg.GCSStateBucket)
			}
		})
	}
}
