...
Preview capability and artifact responses use `Cache-Control: no-store` unless a later privacy specification explicitly defines an encrypted/private cache. The browser fetches the capability without navigating to it, requires exact `image/webp`, validates the RIFF/WebP signature, and constructs a new typed in-memory blob before display. Fast loading is achieved through small variants, direct preview-provider transfer, virtualization, bounded parallelism, and reuse within the current document—not through persistent sensitive browser caches.
